...
<h4 >Rust 中的每一个值都有一个 所有者（owner）。</h4>
<h4 >值在任一时刻有且只有一个所有者。</h4>
<h4 >当所有者离开作用域，这个值将被丢弃。</h4>

#### 变量作用域

在所有权的第一个例子中，我们看看一些变量的 `作用域(scope) {}`。作用域是一个项 在程序中有效的范围，假设有这样一个变量

```rust
let s = "hello";
```

当 s 绑定到一个字符串字面值的时候，这个字符串值是硬编码到程序代码中的。这个变量从声明的位置开始直到当前`作用域`结束时，都是有效的。

下面的示例中的注释标明了变量 `s` 在何处时有效的。
...
截至目前为止，变量是否有效宇作用域的关系和其他语言是类似的。后面来看看 `String`类型。